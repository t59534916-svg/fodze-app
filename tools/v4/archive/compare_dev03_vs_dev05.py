"""
compare_dev03_vs_dev04.py — Side-by-side dev-03 vs dev-05 on 25/26 holdout.
...
HOLDOUT = REPO_ROOT / "tools" / "backtest" / "odds-close-24-25.parquet"
...
def run_for_tag(tag, df_holdout, history):
    ...
    # Ensure disagreement calculator is fitted (auto-fits in _get_disagreement())
    if hasattr(pred, "_get_disagreement"):
        pred._get_disagreement()
    ...
    # Pass disagreement calculator (dev-05+)
    if hasattr(pred, "_get_disagreement"):
        features_kwargs["disagreement_calculator"] = pred._get_disagreement()
    ...
    # Attach per-match disagreement value (looked up from feature row)
    # for downstream slicing
    disagreement_per_match = features.get("market_disagreement_flag",
                                          pd.Series([0.0] * len(features)))
    rows = []
    ...
    for i, row in df_h.iterrows():
        ...
        for outcome_idx, (label, col) in enumerate(
            ...
            rows.append({
                ...
                "edge": edge,
                "won": 1 if won else 0,
                "disagreement_flag": float(disagreement_per_match.iloc[i]),
            })
...
def main():
    print("=" * 70)
    print("dev-03 vs dev-05 — Side-by-side Holdout Comparison")
    print("=" * 70)
    ...
    delta = results['dev-05']['brier'] - results['dev-03']['brier']
    sign = "✓ better" if delta < 0 else ("✗ worse" if delta > 0 else "= same")
    print(f"    Δ (dev-05 − dev-03): {delta:+.4f}  {sign}")
    ...
    # ─── High-disagreement-specific ROI ───
    print(f"\n  ROI on bets where market_disagreement_flag > 0.08 (the target subgroup):")
    print(f"  {'tag':<12} {'subset':<28} {'n':>5} {'win%':>7} {'ROI%':>9} {'CI lo':>9} {'CI hi':>9}")
    print("  " + "─" * 84)
    for tag in TAGS:
        df_dec = results[tag]["df_dec"]
        # All positive-edge bets where the match had high disagreement
        sub_high = df_dec[(df_dec["edge"] > 0) & (df_dec["disagreement_flag"] > 0.08)]
        sub_low = df_dec[(df_dec["edge"] > 0) &
                         (df_dec["disagreement_flag"] <= 0.08) &
                         (df_dec["disagreement_flag"] > 0.0)]  # has odds, low disag
        sub_no_odds = df_dec[(df_dec["edge"] > 0) &
                             (df_dec["disagreement_flag"] == 0.0)]
        for sub, label in [(sub_high, "high_disag (>8%)"),
                           (sub_low,  "low_disag  (≤8%, w/ odds)"),
                           (sub_no_odds, "no_odds_for_match")]:
            r = subgroup_metrics(sub, label)
            if r["n"] == 0:
                continue
            print(f"  {tag:<12} {r['label']:<28} {r['n']:>5} "
                  f"{r['win_rate']*100:>6.2f}% {r['roi']*100:>+8.2f}% "
                  f"{r['roi_lo']*100:>+8.2f}% {r['roi_hi']*100:>+8.2f}%")

    # ─── Correlation of disagreement vs profit (the metric we wanted to fix) ───
    print(f"\n  Correlation of disagreement_flag vs profit-per-stake (the negative-corr target):")
    from scipy.stats import pearsonr
    for tag in TAGS:
        df_dec = results[tag]["df_dec"]
        pos = df_dec[df_dec["edge"] > 0].copy()
        # Compute per-bet profit-per-stake
        pos["stake"] = [
            _kelly_stake(e, o) for e, o in zip(pos["edge"], pos["decimal_odds"])
        ]
        pos["profit"] = np.where(pos["won"] == 1,
                                  pos["stake"] * (pos["decimal_odds"] - 1.0),
                                  -pos["stake"])
        # Only matches with odds (disagreement_flag > 0)
        with_odds = pos[pos["disagreement_flag"] > 0.0]
        if len(with_odds) < 10:
            print(f"    {tag:<12} → too few odds-having decisions ({len(with_odds)})")
            continue
        # Per-stake-unit (avoids stake-size confound)
        profit_per_stake = with_odds["profit"] / with_odds["stake"].clip(lower=1e-9)
        r, p = pearsonr(with_odds["disagreement_flag"].values, profit_per_stake.values)
        print(f"    {tag:<12} → corr(disag_flag, profit/stake) = {r:+.4f} (p={p:.4f}, "
              f"n={len(with_odds)})")

    print(f"\n  SHAP on dev-05 features (HC vs LC matches):")
    res = results["dev-05"]
    ...
    for f, hc_v, lc_v in feat_sorted:
        if lc_v > 1e-12:
            ...
            ratio_str = "inf"
        marker = ""
        if f in ("lineup_quality_diff", "form_streak_diff"):
            marker = " ← dev-03"
        if f in ("market_disagreement_flag", "market_disagreement_high"):
            marker = " ← dev-05 NEW"
