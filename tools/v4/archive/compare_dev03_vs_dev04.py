"""
compare_dev03_vs_dev04.py — Side-by-side dev-03 vs dev-04 on 25/26 holdout.
...
def run_for_tag(tag, df_holdout, history):
    ...
    # Pass disagreement calculator (dev-04+)
    if hasattr(pred, "_get_disagreement"):
...
def main():
    print("=" * 70)
    print("dev-03 vs dev-04 — Side-by-side Holdout Comparison")
    print("=" * 70)
    ...
    delta = results['dev-04']['brier'] - results['dev-03']['brier']
    sign = "✓ better" if delta < 0 else ("✗ worse" if delta > 0 else "= same")
    print(f"    Δ (dev-04 − dev-03): {delta:+.4f}  {sign}")
    ...
    print(f"\n  SHAP on dev-04 features (HC vs LC matches):")
    res = results["dev-04"]
