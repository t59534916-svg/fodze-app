...
**~~Known JS↔TS canonical inconsistency (2026-04-29)~~ — RESOLVED (verifiziert 2026-05-29):** Der alte `dedupe-team-names.mjs::buildAliasMap`-Bug existiert nicht mehr — das Script nutzt jetzt `sharedCanonicalize` (aus canonical-team.mjs) als single source of truth, `findCanonical` nur als matchday-derived Fallback. Zusätzlich wurde 2026-05-29 die zuvor diagnostizierte EXTRA_ALIASES JS↔TS-Desync gefixt, und am 2026-05-31 nach einem full-DB-Sweep um +58 cross-source-fragmentation-Aliase erweitert: beide Files (`canonical-team.mjs` JS + `team-resolver.ts` TS) tragen jetzt **86 identische Einträge**, verifiziert mit residual-fragmentation=0 + JS↔TS-Parität=0 über alle 22 Ligen / alle distinct DB-Teams. Sync-Rule-Kommentar in beiden Files aktiv.
...
| `scripts/check-doc-truth.mjs` | Doku-Wahrheits-Gate: Next/React-Major in Docs vs package.json (HARD, CI-blockierend) + Test-Count-Drift (soft warn, lower-bound-tolerant). Node-builtin only. | `npm run doc-truth` / CI |
| `scripts/bench-dev03-predict.mjs` | dev-03 Forward-Pass Micro-Benchmark gegen echtes Model. Misst per-Match-Kosten (~0.46ms warm — die Worker-„15ms"-Prämisse war ~30× überzogen; einzige echte Latenz ist der 7.6MB-Model-Parse ~75ms). | On demand |
...
| `canonical-team.mjs` | `canonicalize(team, league)` — single source of truth für ingest-side. TEAM_REGISTRY (354 entries from team-resolver.ts) + EXTRA_ALIASES (86 lower-tier overrides). Mirror in `src/lib/team-resolver.ts::canonicalizeTeamName` für read-side (JS↔TS synced; full-DB-residual=0 verifiziert 2026-05-31) |
...
Next.js 16 App Router (React 19, alle pages "use client")
  ...
  │      engine-pick.ts    ← pickPrimaryCalc: hot-path engine selection + ensemble-fallback (extrahiert aus MatchdayContext, getestet)
  │      dev03-overlay-merge.ts ← mergeDev03Overlay/resolveOverlay: async dev-03 Worker-Fold + Blend-Rebuild, stale-tag-guard (getestet)
  │      matchday-cache.ts ← engineCacheKey/engineCacheVersionKey/favouredSide/parseOddsMap: Cache-Korrektheit (byte-identisch zum alten Inline-Format, getestet)
  │      selective-prediction.ts ← convictionForPick/selectHighConviction: validierte-Confidence-Tier-Selektion (≥65%→~73%), Markt-Konsens nur als optionaler Filter
  │
  ├── API-Routes
...
**Maintenance:** `scripts/dedupe-team-names.mjs` und `scripts/dedupe-team-metadata.mjs` sind idempotent re-runnable. Bei neuen Aliasen die im Cron auftauchen: erst EXTRA_ALIASES erweitern, dann re-run. **Daten-Sicherheit (Fix 2026-05-31):** der UNIQUE-Konflikt-Pfad in `dedupe-team-names.mjs` machte früher einen blind-bulk-DELETE aller Alias-rows und zerstörte damit die *bessere* Datenquelle (z.B. Sofa-xG), wenn die Canonical-row aus einer schwächeren Quelle kam. Jetzt `mergeConflictingRows` per-(date,venue) source-priority (`sofascore > understat > api-sports > footystats > shots-model > goals-proxy`, pure `pickWinner`/`sourcePriority` getestet). **Hinweis:** der read-side Alias-Pfad (MatchdayContext canonicalisiert vor dem Bucketing) merged Fragmentierung schon at read-time — DB-Dedup ist optionale Aufräumung, nicht nötig für korrekte Engine-Reads.
...
- `engine-pick.test.ts` — Hot-Path Engine-Selektion + ensemble-Fallback (pickPrimaryCalc, dev-03-Overlay-Race, no-mutation)
- `dev03-overlay-merge.test.ts` — async dev-03 Fold + Blend-Rebuild, **Regression-Test für stale cross-matchday pairing** (Tag-Guard)
- `matchday-cache.test.ts` — Engine-Cache-Keys + **byte-identische Äquivalenz** zum alten Inline-Format (Schutz gegen silent cache-key drift)
- `selective-prediction.test.ts` — validierte-Confidence-Tier-Selektion + Markt-Ehrlichkeits-Invariante (Markt ändert nie Level/Hit-Floor)
- `dedupe-source-priority.test.ts` — `pickWinner`/`sourcePriority` (Sofa-xG-Alias schlägt goals-proxy-Canonical, Ties→canonical)
- `canonicalize-team-name.test.ts` — JS↔TS-Mirror inkl. der +58 fragmentation-Aliase (rollover-Fix 2026-05-31)
...
| Area | Status | Notes |
|---|---|---|
| **MatchdayContext hot-path tests** | live (2026-05-31) | Die ungetestete 1000-LOC-Context-Logik (SWOT-Schwäche) wurde an den bug-anfälligsten Stellen in pure, getestete Module extrahiert: `engine-pick.ts` (Selektion+Fallback), `dev03-overlay-merge.ts` (async-Fold + **stale-pairing-Regression**), `matchday-cache.ts` (Cache-Keys + byte-identische Äquivalenz). Reine Refactors — Verhalten provably unverändert. |
| **dev-03 Worker = nicht load-bearing** | measured (2026-05-31) | `scripts/bench-dev03-predict.mjs` misst ~0.46ms/Match warm — die Worker-Begründung „~15ms/Match blockiert React" war **~30× überzogen**. Ein 40-Match-Tag = ~19ms dev-03-Compute. Einzige echte Latenz: einmaliger 7.6MB-Model-JSON-Parse (~75ms), den der Worker NICHT entfernt (beide Threads parsen). Worker behalten (harmlos, Sync-Fallback trägt Tests); überzogene Kommentare korrigiert. Exakte Browser-Zahl braucht V8-Profiling unter Live-Last (Saisonstart). |
| **Doku-Wahrheits-Gate** | live, CI-blockierend (2026-05-31) | `scripts/check-doc-truth.mjs` als blockierender CI-Schritt: Next/React-Major in jeder Doc MUSS package.json entsprechen (HARD); Test-Count-Drift soft-warn (lower-bound-tolerant gegen parametrisierte Loops). Adressiert die als „größtes Risiko" geflaggte Stale-Doku-Schwäche an der Wurzel. Fing sofort 3 Docs mit veralteten Framework-Versionen (Next/React eine Major zu niedrig). `npm run doc-truth`. |
...
| **Per-engine calibration bypass (Kelly track)** | live in production — merged `3aaf2b4` → on `main` `2df7271` (verified 2026-06-01; the earlier "UNPUSHED" was stale) | The shared global isotonic in `public/calibration_curves.json` was fit on the ensemble/Dixon-Coles DISPLAY distribution (header "14.359 games 2017-2025"; git "Train Dixon-Coles calibration on real xG"). Measured via the REAL production `calibrate1X2` (`tools/backtest/engine_calibrated_brier.mts` → `validate_calibration_bypass.py`): the curve **HELPS Standard** (Brier 0.682→0.651, ECE 0.151→0.052 — it owns that dist) but **HURTS v1/v2/dev-03 on BOTH Brier AND ECE** (v2 0.624→0.652 / ECE 0.013→0.058; dev-03 0.621→0.650 / ECE 0.021→0.063). Both axes worsening ⇒ NOT a sharpness↔reliability trade, the curve is mis-applied. **Cross-season confirmed** (the incumbent's exact failure was cross-season): dev-03 24/25-OOT (dev-03-2h) shows the same pattern — raw BrierΔ −0.027 / ECEΔ −0.040, blended BrierΔ −0.022 / ECEΔ −0.010, near-identical to 25/26. **Fix:** `bypassSharedCalibration(engine)` in `src/lib/calibration.ts` — `{v1,v2,dev-03}` skip the shared isotonic on the Kelly/edge track (`calculateBetsEnhanced` pModel + `dualTrackCalibrate` Track B = identity); ensemble keeps it. **1X2 only** — O25 isotonic untouched (per-engine effect unmeasured). Bypass, not refit: v2/dev-03 raw ECE already 1.3%/2.1% → nothing to refit + a fresh fit would re-introduce the single-season overfit that made the stale curve. 5-Gate: G1-G4 pass (all 4 deltas Holm-sig, powered n≥6.5k); G5 Money-Eval ROI "worsening" under bypass is NOT robust (2000-sample bootstrap profit/match Δ CI crosses 0, `_bootstrap_roi_delta.py`) — edge vs Pinnacle is validated-impossible (§5b) so the goal is forecast quality, which bypass strictly improves. Display probs + confidence badge UNCHANGED (v2/dev-03 display was already raw/Benter-blended, never isotonic). 9 wiring tests in `tests/calibration-engine-bypass.test.ts`; full sweep 902/902 + tsc 0. **Merged** into the dev-03-default line at `3aaf2b4` (the dev-03-default commit `b704ec5` had bundled an early draft of the harness; merge conflicts resolved to the evolved fix-branch versions). **Conformal coupling — RESOLVED (commit 81250ed):** `calculateBetsEnhanced` feeds the 1X2 conformal gate `calIso` (always the calibration-layer output) instead of the bypass-aware `cal`. Quantiles were fit on CALIBRATED probs (`conformal-quantiles.json` `_meta.calibration`), so this restores the gate's exact pre-bypass input — a future `NEXT_PUBLIC_CONFORMAL_GATE=enforce\|dampen` flip behaves identically for v1/v2/dev-03 with **no per-engine quantile refit**. No-op today (gate=`warn`→1.0); non-bypass engines have `cal===calIso`. **Update 2026-06-01 (PR #15, merged `2df7271`):** the shipped quantiles were dirichlet-fit and under-covered the real isotonic+Benter runtime (v2-OOT n=6525: α=.05 −3.5pp, α=.10 −3.3pp) — re-fit on `calibrate1X2(benterBlend(raw))` (`runtime-isotonic-benter`, coverage restored +0.3pp both α). `enforce` then evaluated as a STAKING policy and REJECTED (skips 96.4% of 1X2 bets, favourite-biased, KEPT−SKIPPED ROI-gap CI crosses 0 vs Pinnacle closing — `conformal_enforce_eval.py`/`conformal_roi_eval.py`). Gate stays `warn`. Guard: `tools/backtest/conformal_gate_input_check.mts` (kept a `.mts` harness, not `tests/*.test.ts`: `calculateBetsEnhanced` under an active conformal mode trips a vitest worker-pool quirk only at full-suite scale; the gate fns are pure → infra issue, not a code bug). **Calibration default HARDENED (commit 2c3b78b):** AppContext + `/health` default `NEXT_PUBLIC_CALIBRATION_METHOD` `"dirichlet"`→`"isotonic"`. `dirichlet-calibration.json` ships, so an env-var-dropped deploy would silently load it + flip production to the REVERTED-worse method (drift +0.0075). Unset fallback now = validated method; production unchanged (env explicitly isotonic). **Platt-params note:** `calibration_curves.json` is `method:"platt"` w/ 18 dormant per-league param sets (env=isotonic → runtime uses hardcoded global `CAL_H/D/A`); left as-is (retrain_all.py owns the file; bypass already shields v1/v2/dev-03 from any platt flip); misleading AppContext comments corrected. |
| **Forecast-quality framework + Blend** | analyzed (2026-05-28), NOT wired | Objective-Schwenk: Prognose-Güte (xG-RMSE + 1X2-Brier gekoppelt), ROI nur sekundärer Tiebreaker. `v4.eval.metrics` xG-forecast-Primitive (xg_rmse/mae/bias, 6 tests) + 21 Diagnostics in `tools/v4/diagnostics/`. **50/50 λ-Blend (dev-03⊕dev-09) dominiert beide Reinmodelle auf BEIDEN Achsen, BEIDEN Holdouts** (25/26: 0.7016/0.6111 · 24/25: 0.6873/0.6093 = xG-RMSE/Brier) — validiert-bester Forecaster, aber **NICHT wired** (dev-09 braucht Live-Lineup-Pipeline; dev-03 bleibt Production-Default). Scorecard 25/26 OOT: 1X2 48.9%, O/U 55.2%, xG-MAE 0.53 Tore, BSS +5.9%. **Confidence-Badge auf ECHTEM Production-Pfad validiert** (`validate_confidence_production_path.py`, 2026-05-29): das Badge zeigt dev-03 **Benter-geblendet** Richtung Pinnacle (NICHT Isotonic — das ist Track-B/Kelly-only); Blend verbessert Brier 0.619→0.604 → HOCH ≥65% trifft 78.7% (25/26) / 73.5% (24/25 OOT), die „~73%"-Claim ist konservative Untergrenze. Single-source `src/lib/confidence-tier.ts` (12 Tests). **xG-Skill-Score** vs Liga-Mittel-Klimatologie (RMSE 0.733, `xg_skill_baseline.py`): +4.2% dev-03 / +8.4% Blend → echtes aber bescheidenes Skill, 0.70 RMSE ist größtenteils irreduzibles per-Spiel-Rauschen. **REJECTED (5-Gate/Persistenz):** dominance-conversion (redundant mit dev-03-λ, Residual r=0.024, Brier-Δ 1.1σ, G5 negativ) · draw-value (25/26 +5.9% CI⊃0, 24/25 OOT −8.16% CI<0 = Rauschen) · training-focus-on-high-conf (falsifiziert über k-Sweep {0,0.5,1,2,4} `eval_conf_weight_sweep.py`: KEIN monotoner High-Conf-Gewinn — einziger Dip bei k=0.5 −0.0093 ist nicht-monoton = Rauschen, Overall-Brier monoton schlechter mit k, High-Conf-Acc fällt 72.6→71.6%; High-Conf ist nur ~7% des Brier-Verlusts + Upset-gedeckelt → richtiger Hebel = selektive Vorhersage). Neue Fähigkeit: `BayesianEnsemble.fit(sample_weight=)` + `train_m3_xg --conf-weight-k` (additiv, default-aus, getestet). **Markt-Head-to-Head (2026-05-29, `analyze_pick_quality.py` + `analyze_ou_vs_market.py`):** Production-Pfad (dev-03 Benter-blended) schlägt Pinnacle WEDER auf 1X2 NOCH auf Ü/U — beide verlieren Brier + den Disagreement-Test (wo wir abweichen, trifft der Markt häufiger: 1X2 26.6% vs 44.3%, Ü/U 46.7% vs 53.3%), Ü/U-Flat-Stake-ROI vs Closing −1.7%/−4.1%, per-Liga-Ausreißer persistieren NICHT cross-season (= Rauschen, nicht Edge — frühere „ligue_1/la_liga O/U-Edge"-Behauptung damit falsifiziert). **Methodik-Lektion:** Backtest-Parquets sind 1X2-vollständig aber Ü/U-arm (~1%) — Markt-Ü/U-Analysen MÜSSEN `odds_closing_history` lesen (80% psc_over25, 24.617 Zeilen), NICHT das Parquet. Kein Wett-Edge auf den pickbaren Märkten — bestätigt. Vollbericht: `docs/FORECAST-QUALITY-ANALYSIS.md`. |
...
| Area | Status | Notes |
|---|---|---|
| **Conformal Gate drift** | **RE-FIT 2026-06-01 on REAL runtime dist 🟢 — the 2026-05-28 fit was validated on the WRONG (Dirichlet) distribution** | Re-fit on 25/26 OOT (n=6,525, 2025-08 → 2026-05) via `tools/backtest/refit-all.sh`. Root cause was stale hardcoded date window in `tools/calibrate_dirichlet.py:170` + `tools/fit_conformal.py:196` (was `2023-08-01...2024-07-01` since dev-02 era; advanced to `2025-08-01...2026-07-01`). **Per-league empirical coverage on Dirichlet-CALIBRATED probs (apples-to-apples with old audit): ALL 18 leagues \|drift\| ≤ 0.73pp.** EPL: **+0.24%** (was -8.50% catastrophic on Dirichlet path). Worst: scottish_prem +0.73%. Aggregate at α=0.05 = +0.19% (near-perfect). Brier 0.6267 → 0.6227. **CORRECTED 2026-06-01:** that coverage was measured on Dirichlet-calibrated probs — but prod serves isotonic+Benter, so the shipped quantiles UNDER-cover the real runtime (v2-OOT n=6525: α=.05 −3.5pp, α=.10 −3.3pp). Re-fit on the real `calibrate1X2(benterBlend(raw))` distribution via option B (`_conformal_export_raw.py`→`conformal_runtime_calibrate.mts`→`refit_conformal_runtime.py`, PR #15): coverage restored (+0.3pp both α; flip_safe reproduced on main). `enforce` then evaluated as a staking policy and REJECTED (skips 96.4%, favourite-biased, no robust ROI benefit — `conformal_enforce_eval.py`/`conformal_roi_eval.py`). **Gate STAYS warn.** Previous drift audit `tools/backtest/conformal-drift-report.json` (2026-04-29) superseded. Walk-forward isotonic-per-league attempt 2026-05-22 still archived in `tools/v4/modules/m10_per_league_calibration/` (it failed, but kept for multi-season validation reference). |
...
| **`sofascore_team_rolling_8` MATERIALIZED view** | RESOLVED 🟢 (materialized 2026-05-28, verifiziert 2026-05-31) | War regular VIEW mit Full-Scan + window-aggregation (~1.7s service-key / ~3s anon timeout). **Jetzt MATERIALIZED VIEW** (`scripts/migration-sofascore-team-rolling-8-materialized.sql`): GROUP BY team_id only (de-dupt 5 name-drift teams), UNIQUE INDEX (league,season,team_id) für REFRESH CONCURRENTLY + covering index auf (league,season,team). **Gemessen 2026-05-31 auf live DB: Index-Scan-Query 8.4ms (war ~1.7-3s), 1614 rows × 22 Ligen × 4 Saisons, 472 kB.** Refresh via `public.refresh_team_rolling_8()` RPC (SECURITY DEFINER, statement_timeout=60s, ~14s e2e) aufgerufen von `scripts/refresh-rolling-8.mjs` als Phase `rolling-8-refresh` in refresh-all.mjs nach der shotmap-sync. **Read-Pattern**: JOIN by team_id bevorzugt (stabil); (league,season,team) für text-name-fallback indexiert — 5 name-drift teams (Liverpool FC/Milan/Roma/SSC Napoli/Deportivo La Coruña) brauchen team_id ODER canonical_team() vor dem text-lookup. Retrain-Iteration: vorher ~11min/rolling-8-load → jetzt Sekunden. |
...
| **Conformal Gate (Phase 2.5)** | **warn — STAYS warn (enforce evaluated 2026-06-01 → rejected; quantiles re-fit on real runtime dist, PR #15)** | `public/conformal-quantiles.json` (trained 2026-05-28 on 25/26 OOT n=6,525) | Empirische Coverage 18/18 leagues \|drift\| ≤ 5pp on fresh fit. EPL +1.36% (was -8.50%). Aggregate α=0.05 = +0.19%. **CORRECTION 2026-06-01:** the 2026-05-28 coverage was measured on DIRICHLET-calibrated probs, but production serves isotonic+Benter → those shipped quantiles UNDER-cover the real runtime (measured v2-OOT n=6525: α=.05 0.915/−3.5pp, α=.10 0.867/−3.3pp). Re-fit on the real `calibrate1X2(benterBlend(raw))` distribution (PR #15, `runtime-isotonic-benter`): coverage restored (α=.05 +0.3pp, α=.10 +0.3pp; repeated-split CV no robust per-league under-coverage). Separately, `enforce` was evaluated as a STAKING policy and REJECTED — it skips 96.4% of 1X2 bets (favourite-biased, mean top-prob 0.76) with NO robust ROI benefit (KEPT−SKIPPED ROI-gap CI crosses 0 at every edge threshold vs Pinnacle closing). Gate STAYS warn. Refit/eval: `refit_conformal_runtime.py` + `conformal_enforce_eval.py` + `conformal_roi_eval.py`. Refit chain: `tools/backtest/refit-all.sh`. |
