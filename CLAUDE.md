...
**Sofascore Shot-Event Pipeline (NEU 2026-04-29, Tier-B-erweitert 2026-05-03)**: ~141k per-shot events in `sofascore_shotmap` × ~5.7k matches in `sofascore_match` (17 von 22 Ligen × Saison 25/26). Datenquelle: [`datafc`](https://pypi.org/project/datafc/) (curl_cffi mit Chrome 124 TLS-Fingerprint, kein Browser nötig). Pro Shot: xG, xGOT, body_part, situation (assisted/corner/fast-break/penalty/...), shooter coords, outcome. Drei Views: `sofascore_team_chance_quality` (per-game chance-quality), `sofascore_team_rolling_8` (last-8-games per team — engine-input shape), `sofascore_standings` (live league table). `sofascore_data_quality_tier(league)` SQL-Funktion klassifiziert in `premium` (11 Ligen mit voll xG + assisted/fast-break tags: bundesliga, bundesliga2, championship, epl, eredivisie, greek_sl, la_liga, ligue_1, primeira_liga, serie_a, serie_b) / `partial` (liga3 — voll xG aber ohne assisted/fast-break tags) / `volume` (5 Ligen ohne xG: eerste_divisie, la_liga2, league_one, league_two, ligue_2). Tier-Klassifikation aktualisiert via `scripts/migration-sofascore-tier-update.sql` (2026-05-03). Sync läuft als Phase 4 in `refresh-all.mjs` via `scripts/sync-sofascore-shotmap.mjs`.
...
| **Tier-B Sofascore backfill** | 🟡 partial (2026-05-03) | 6 von 11 Tier-B-Ligen in `sofascore_match` geladen: league_one (495 m), league_two (485 m), eredivisie (292 m), eerste_divisie (393 m), primeira_liga (290 m), greek_sl (29 m). 3 davon haben echtes xG (eredivisie/primeira_liga/greek_sl → premium tier), 3 haben nur shot-events ohne xG (league_one/league_two/eerste_divisie → volume tier — Sofascore-Upstream-Limit). Noch fehlend: jupiler_pro (tid=38), super_lig (tid=52), scottish_prem (tid=36), austria_bl (tid=45), swiss_sl (tid=215) — die 5 IDs sind korrekt, aber Cloudflare returnte HTTP 403 "challenge" auf den seasons-list endpoint nachdem die ersten 6 Ligen erfolgreich gefetcht wurden (kumulatives rate-limiting). Re-try der 5 nach 24-72h IP-cooldown via z.B. `node scripts/sync-sofascore-shotmap.mjs --league jupiler_pro --pace 4.0`, sequentiell mit 30+ Min Pause zwischen Ligen. |
