...
**Sofascore Post-Match Extras Pipeline (v1 seit 2026-05-07, v2 seit 2026-05-08)**:
**v1** (4 endpoints, 736 games covered): `sofascore_match_statistics` (4256 rows, ~40 team stats × period=ALL/1ST/2ND), `sofascore_player_match_stats` (29.5k rows, per-player rating/xA/key_passes/touches_in_box/goals_prevented), `sofascore_incidents` (15k goal/card/sub timeline rows), `sofascore_average_positions` (22.7k tactical avg-pitch coords). Plus `sofascore_extras_state` sync-tracker (forever-cache nach status='Ended'). 18 Feature-Columns auf `team_xg_history` befüllt via `scripts/bridge-sofascore-extras-to-team-xg.mjs` (PATCH-only, 890 rows mit big_chances/possession/tackles/cards). Pipeline: `tools/sofascore/{fetch_match_extras,load_extras_to_supabase}.py` orchestriert via `scripts/sync-sofascore-extras.mjs` als Phase 6 in refresh-all (opt-in `--extras` flag).
**v2 HIGH-SIGNAL** (3 endpoints, applied 2026-05-08): `sofascore_match_managers` (manager_id is stable key for coaching-change detection — replaces manual NEUER-TRAINER tag in `/api/matchday`), `sofascore_pregame_form` (Sofa's pre-match form summary: avgRating + position + last-5), `sofascore_team_streaks` (~13/game general + head2head). Migration `scripts/migration-sofascore-event-extras-v2.sql` + view `sofascore_team_manager_history`. **NEUER-TRAINER auto-detection** via `scripts/_lib/matchday-enrich.mjs::deriveCoachingChangeTag` (16 vitest cases). **Cloudflare reality:** v2 endpoints werden direkt 403'd seit 2026-05-07 — alle Aufrufe brauchen Tor SOCKS5 (chrome124 + Tor exit IP umgeht Cloudflare-Inspection). Setup: `brew install tor && brew services start tor`. Aktivierung: `--use-tor` flag oder `FODZE_EXTRAS_USE_TOR=1` env. End-to-end empirisch verifiziert (Spiel 14035865 → 17 rows in 3 Tabellen, 0 errors). Bulk-backfill (6810 pending) ist aber Cloudflare-vs-Tor cat-and-mouse — incremental cron + paid residential proxy sind die zwei realistischen Pfade.
...
## Areas to Watch (Stand 2026-05-09)
...
| **Sofascore extras v2 (HIGH-SIGNAL endpoints)** | ✅ schema + pipeline shipped (2026-05-08), ⚠ backfill blocked | 3 neue Tabellen + view: `sofascore_match_managers`, `sofascore_pregame_form`, `sofascore_team_streaks` + `sofascore_team_manager_history` view. End-to-end empirisch verifiziert (Spiel 14035865 → 7/7 endpoints → 17 rows). NEUER-TRAINER auto-tag via `deriveCoachingChangeTag` (16 vitest cases). **Cloudflare blockt direkt seit 2026-05-07 — Tor SOCKS5 (`brew install tor; --use-tor` flag) erforderlich**. Smoke-test 2026-05-08 mittag zeigt aggressive Tor-exit blocks (3/3 games HTTP 403 nach kurzer Erfolgsphase). Bulk-backfill via Tor unzuverlässig; realistische Pfade: incrementeller cron mit `FODZE_EXTRAS_USE_TOR=1` (täglich ~25 games wenn Tor klappt) ODER residential proxy ($5-10/Mo). |
