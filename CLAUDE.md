...
| **Money-Eval Hybrid-Per-League Map** | UPDATED (2026-05-25) — policy rewritten under 5-Gate Falsification | **NEW dev-03 (multi-season corpus 2022-07→2025-08):** Stage 5 on 25/26 holdout — €100→€149.14 (+49.1% compound), ROI +5.4%/bet. **WALK-FORWARD validation** (train 22/23+23/24, holdout 24/25): Brier 0.6128 (Δ=-0.0065). Money-Eval 24/25: ROI +3.36%, max-DD 24.22%. Per-Liga ROI rankings are HIGHLY UNSTABLE between holdouts. **2026-05-25 audit (`tools/v4/diagnostics/bet_edge_policy_audit.{py,json}`)** applied 5-Gate Falsification Protocol on the previous 5-league policy: 3 of 5 'validated' leagues failed cross-validation (epl -34.5%, serie_a -14.5%, serie_b -13.1% on 24/25 walkfwd — catastrophic reversals). **NEW policy (commit 21a13c1)**: 4 Holm-Bonferroni-survivors, all `engine='dev-03'`: la_liga (mean +36.27%, p_adj=0.000, n=105), scottish_prem (+36.17%, p_adj=0.000, n=50), bundesliga (+30.15%, p_adj=0.003, n=42, NEW), primeira_liga (+27.32%, p_adj=0.003, n=53, NEW). Schema migrated: roi23_24/roi25_26 → roi_walkfwd_24_25/roi_holdout_25_26 + added holm_p_adj column. **Public API unchanged** (`validatedEngineFor`, `hasValidatedEdge`, `leagueEdgeRecord`, `expectedROIperStake`, `validatedLeagues`) → Goldilocks page works unchanged. **Caveat**: even Holm-survivors could fail in 26/27 — re-audit annually via `bet_edge_policy_audit.py`. Aggregate dev-03 itself fails 2/4 Stage 5 ship-gates (bootstrap CI lower bound includes 0 in both holdouts), so this policy is the RISK-MITIGATION layer — restrict betting to the 4 leagues with strongest statistical evidence. Audit-Sims: `tools/v4/reports/stage_5_kelly_clv_dev-03_dev-03_M_α1.0.json` (25/26) + `..._walkfwd_dev-03-walkfwd_M_α1.0.json` (24/25). |
...
player_season_stats — FootyStats CSV import (NEU 2026-05-25). 34.216 player-season rows,
...
match_prematch_signals — FootyStats CSV value-adds (NEU 2026-05-25). 38.696 rows,
